...
use App\Http\Requests\helpdesk\TicketSourceUpdateRequest;
// models
...
class TicketSourceController extends Controller
{
    ...
    {
        try {
            $ticketSources = Ticket_source::orderBy('id', 'desc')->get();
            
            return view('themes.default1.admin.helpdesk.settings.ticket-source.index', compact('ticketSources'));
        } catch (Exception $ex) {
    ...
    {
        try {
            $ticketSource = Ticket_source::whereId($id)->first();

            return view('themes.default1.admin.helpdesk.settings.ticket-source.edit', compact('ticketSource'));
        } catch (Exception $ex) {
            return redirect()->back()->with('fails', $ex->getMessage());
        }
    }

    public function update(TicketSourceUpdateRequest $request, $id)
    {
        $ticketSource = Ticket_source::findOrFail($id);

        $ticketSource->name = $request->input('name');
        $ticketSource->value = $request->input('value');
        $ticketSource->css_class = $request->input('css_class');
        $ticketSource->status = $request->input('status');
        ...
        return redirect()->route('ticket-source.index')->with('success', Lang::get('lang.ticket-source.msg-successfully_update'));
    }
